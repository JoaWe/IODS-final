...
```{r LRA Life-Exp & MMR, fig.height=4, fig.width=6}
...
The basis of multiple regression is the same as linear regression: An independent variable is drawn upon, in order to explain a dependent variable.

The difference, however, is that in this case it is not a linear but an exponential function that is sought in order to provide the explanation.
